...
CFontHandle CGridCell::GetFontObject() const
{
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		m_pEditWnd = new CInPlaceEdit(pGrid->m_hWnd, rect, dwStyle, nID, nRow, nCol, GetText(), nChar, GetCType());
    }
    return TRUE;
}
...
#endif
    ATLVERIFY(SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &ncm, 0));
...
void CGridDefaultCell::SetFont(const LOGFONT* plf)
{
    ...
    if (!DC.IsNull())
    {
		...
		bRes = DC.GetTextExtent(_T(" "), 1, &m_Size);
		ATLASSERT(bRes);
        SetMargin(m_Size.cx);
		bRes = DC.GetTextExtent(_T(" XXXXXXXXXXXX "), 14, &m_Size);
		ATLASSERT(bRes);
...
{
    ATLASSERT(m_plfFont);  // This is the default - it CAN'T be NULL!
    return m_plfFont;
}
