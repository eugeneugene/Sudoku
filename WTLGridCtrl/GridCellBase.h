...
#define GVIF_CTYPE              (GVIF_STATE<<6)
#define GVIF_ALL                (GVIF_TEXT|GVIF_IMAGE|GVIF_PARAM|GVIF_STATE|GVIF_BKCLR|GVIF_FGCLR|GVIF_FORMAT|GVIF_FONT|GVIF_MARGIN|GVIF_CTYPE)
...
public:
	...
	virtual CGridCellBase* CreateObject() const             = 0 ;
	virtual LPCTSTR IsTypeOf() const						{ return _T("CGridCellBase"); }
// Attributes
public:
	...
	virtual void SetCType(_gctype_t ctype)					{ m_ctype = ctype; }
    ...
    virtual CFontHandle GetFontObject() const               = 0 ;
    virtual CGridCtrl * GetGrid()       const               = 0 ;
    virtual CWindow   * GetEditWnd()    const               = 0 ;
    virtual UINT        GetMargin()     const               = 0 ;
	virtual _gctype_t   GetCType()		const				{ return m_ctype; }
...
public:
    ...
    virtual BOOL Draw(CDCHandle DC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd = TRUE);
    ...
    virtual CSize GetTextExtent(LPCTSTR str, CDCHandle DC = CDCHandle());
    virtual CSize GetCellExtent(CDCHandle DC);

    // Editing
    virtual BOOL Edit( int /* nRow */, int /* nCol */, CRect /* rect */, CPoint /* point */, 
                       UINT /* nID */, UINT /* nChar */) { ATLASSERT( FALSE); return FALSE;}
	virtual BOOL ValidateEdit(LPCTSTR str);
    virtual void EndEdit() {}

    // EFW - Added to print cells properly
    virtual BOOL PrintCell(CDCHandle DC, int nRow, int nCol, CRect rect);
