...
#define GRIDCTRL_CLASSNAME    _T("WTLGridCtrl")  // Window class name
...
#if !defined(GRIDCONTROL_NO_DRAGDROP)
#	undef GRIDCONTROL_NO_CLIPBOARD     // Force clipboard functions on
#endif

#if !defined(GRIDCONTROL_NO_CLIPBOARD) || !defined(GRIDCONTROL_NO_DRAGDROP)
...
#ifdef _UNICODE
#	define	GRID_CF CF_UNICODETEXT
#else
#	define	GRID_CF CF_TEXT
#endif
...
inline CWindow GetOwner(CWindow wnd)
{
	if (wnd.GetStyle() & WS_CHILD)
		return wnd.GetParent();
	return wnd.GetWindow(GW_OWNER);
}

inline CString GetParentWndText(CWindow wnd)
{
	CString m_strTitle;
	CWindow ParentWnd = wnd.GetParent();

	while (ParentWnd.IsWindow())
	{
		TCHAR *buf;
		size_t len = ParentWnd.GetWindowTextLength();
		if (len > 0)
		{
			buf = new TCHAR[len + 1];
			ParentWnd.GetWindowText(buf, (int)len);
			m_strTitle = buf;
			delete[] buf;
			break;
		}
...
typedef CAtlArray<CGridCellBase *> GRID_ROW;
...
// Messages sent to the grid on drag-n-drops events
#define WM_GRD_DRAGENTER		WM_USER+1
#define WM_GRD_DRAGOVER 		WM_USER+2
#define WM_GRD_DRAGLEAVE		WM_USER+3
#define WM_GRD_DRAGDROP 		WM_USER+4

///////////////////////////////////////////////////////////////////////////////////
// Global Variable
///////////////////////////////////////////////////////////////////////////////////

extern _locale_t GridLocale;
...
struct DropTargetData
{
	...
	DWORD grfKeyState;
	POINTL pt;
	DropTargetData() { ZeroMemory(this, sizeof(DropTargetData)); }
};

class CGridDropTarget : public CIDropTarget
{
	HWND m_hWnd;
	public:
	CGridDropTarget(HWND hWnd, HWND hTargetWnd = nullptr) : CIDropTarget(nullptr == hTargetWnd?hWnd:hTargetWnd), m_hWnd(hWnd)
	{
		if (!SUCCEEDED(::RegisterDragDrop(hWnd, dynamic_cast<LPDROPTARGET>(this))))
			AtlTrace(_T("DragDrop target registration failed\n"));
	}
	virtual bool OnDrop(FORMATETC* pFmtEtc, STGMEDIUM& medium, DWORD *pdwEffect)
	{
		HRESULT hRes = CIDropTarget::DragLeave();
		AtlTrace(_T("OnDrop: Effect: %d\n"), *pdwEffect);
		bool bRes = false;
		if (SUCCEEDED(hRes) && ::IsWindow(m_hWnd))
		{
			DropTargetData data;
			data.pFmtEtc = pFmtEtc;
			data.pMedium = &medium;
			bRes = ::SendMessage(m_hWnd, WM_GRD_DRAGDROP, (WPARAM) &data, (LPARAM) pdwEffect) == NULL ? false : true;
		}
		return bRes;
	}
	...
	{
		HRESULT hRes = CIDropTarget::DragEnter(pDataObj, grfKeyState, pt, pdwEffect);
		if (SUCCEEDED(hRes) && ::IsWindow(m_hWnd))
		{
			DropTargetData data;
			data.pDataObj = pDataObj;
			data.grfKeyState = grfKeyState;
			CopyMemory(&data.pt, &pt, sizeof(POINTL));
			hRes = ::SendMessage(m_hWnd, WM_GRD_DRAGENTER, (WPARAM) &data, (LPARAM) pdwEffect);
		}
		return hRes;
	}
	virtual HRESULT STDMETHODCALLTYPE DragOver(DWORD grfKeyState, POINTL pt, DWORD __RPC_FAR *pdwEffect)
	{
		AtlTrace(_T("DragOver: Old effect: %d\n"), *pdwEffect);
		HRESULT hRes = CIDropTarget::DragOver(grfKeyState, pt, pdwEffect);
		if (SUCCEEDED(hRes) && ::IsWindow(m_hWnd))
		{
			DropTargetData data;
			data.grfKeyState = grfKeyState;
			CopyMemory(&data.pt, &pt, sizeof(POINTL));
			hRes = ::SendMessage(m_hWnd, WM_GRD_DRAGOVER, (WPARAM) &data, (LPARAM) pdwEffect);
			AtlTrace(_T("DragOver: New effect: %d\n"), *pdwEffect);
		}
		return hRes;
	}
	virtual HRESULT STDMETHODCALLTYPE DragLeave(void)
	{
		...
		if (SUCCEEDED(hRes) && ::IsWindow(m_hWnd))
		{
			hRes = ::SendMessage(m_hWnd, WM_GRD_DRAGLEAVE, (WPARAM) nullptr, (LPARAM) nullptr);
		}
		return hRes;
	}
};
...
class CGridCtrl : public CWindowImpl<CGridCtrl>, public CUpdateUI<CGridCtrl>
#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)
	, CPrintJobInfo
#endif
{
...
public:
    ...
    BOOL Create(const RECT& rect, CWindow parent, UINT nID,
...
public:
    ...
    void EnableSelection(BOOL bEnable = TRUE)     { ResetSelectedRange(); UIEnable(ID_EDIT_SELECT_ALL, m_bEnableSelection = bEnable); ResetSelectedRange(); }
...
public:
    ...
    BOOL SetCellType(int nRow, int nCol, CGridCellBase* pRuntimeClass);
    BOOL SetDefaultCellType(CGridCellBase* pRuntimeClass);
...
public:
    ...
    BOOL RedrawCell(int nRow, int nCol, CDCHandle DC = nullptr);
    BOOL RedrawCell(const CCellID& cell, CDCHandle DC = nullptr);
...
///////////////////////////////////////////////////////////////////////////////////
#if !defined(GRIDCONTROL_NO_CLIPBOARD)
public:
    virtual void CutSelectedText();
    virtual CIDataObject *CopyTextFromGrid();
    virtual BOOL PasteTextToGrid(CCellID cell, FORMATETC* pFmtEtc, STGMEDIUM& medium, BOOL bSelectPastedCells = TRUE);

	virtual void EditCut();
	virtual void EditCopy();
	virtual void EditPaste();
	void OnEditCut(UINT uNotifyCode, int nID, CWindow wndCtl)	{ EditCut(); }
	void OnEditCopy(UINT uNotifyCode, int nID, CWindow wndCtl)	{ EditCopy(); }
	void OnEditPaste(UINT uNotifyCode, int nID, CWindow wndCtl)	{ EditPaste(); }
	virtual void EditSelectAll();
	void OnEditSelectAll(UINT uNotifyCode, int nID, CWindow wndCtl)	{ EditSelectAll(); }
#ifndef GRIDCONTROL_NO_RTF
	static CString EncodeRTF(const CString &strRTF);
#endif
#endif
...
private:
	...
	CCellID		m_DragEnterCell;
	CGridDropTarget *m_pDropTarget;
public:
...
#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)
public:
	bool Print(CPrinter hPrinter, DEVMODE* pdm = nullptr);
...
protected:
	virtual void PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect, HDC hDC, BOOL& bFirst);
    virtual void PrintColumnHeadings(UINT nPage, HDC hDC);
    virtual void PrintHeader(UINT nPage, HDC hDC);
    virtual void PrintFooter(UINT nPage, HDC hDC);
    virtual void PrintRowButtons(UINT nPage, HDC hDC);

	///////////////////////////////////////////////////////////////////////////////////
	// Printing variables
	///////////////////////////////////////////////////////////////////////////////////
	CFont	m_PrinterFont;
	CFontHandle	m_PrintOldFont;
...
protected:
    ...
    void ShowScrollBars(int nBar, BOOL bEnable = TRUE);
...
protected:
    ...
    // Drawing
    virtual void  OnDraw(CDCHandle DC);
...
protected:
    ...
    CAtlArray<UINT>	m_arRowHeights, m_arColWidths;
    int         m_nVScrollMax, m_nHScrollMax;

    // Fonts and images
    CGridCellBase*	 m_pRtcDefault;  // determines kind of Grid Cell created by default
	...
	CAtlMap<DWORD, CCellID> m_SelectedCellMap, m_PrevSelectedCellMap;
...
protected:
	...
	HFONT OnGetFont()
	{
		return m_cellDefault.GetFontObject();
	}
    void OnImeChar(WPARAM wCharCode, LPARAM nCode);
    LRESULT OnEndInPlaceEdit(LPNMHDR pnmh);
...
//EFW - Added ON_WM_RBUTTONUP
		...
		NOTIFY_HANDLER_EX(IDC_INPLACE_CONTROL, GVN_ENDLABELEDIT, OnEndInPlaceEdit)
		...
		if (uMsg == WM_GRD_DRAGENTER)
		{
			SetMsgHandled(TRUE);
			DropTargetData *pData = reinterpret_cast<DropTargetData *>(wParam);
			DWORD *pdwEffect = reinterpret_cast<DWORD *>(lParam); 
			lResult = DragEnter(pData->pDataObj, pData->grfKeyState, pData->pt, pdwEffect);
			if(IsMsgHandled())
				return TRUE;
		}
		if (uMsg == WM_GRD_DRAGOVER)
		{
			SetMsgHandled(TRUE);
			DropTargetData *pData = reinterpret_cast<DropTargetData *>(wParam);
			DWORD *pdwEffect = reinterpret_cast<DWORD *>(lParam); 
			lResult = DragOver(pData->grfKeyState, pData->pt, pdwEffect);
			if(IsMsgHandled())
				return TRUE;
		}
		if (uMsg == WM_GRD_DRAGLEAVE)
		{
			SetMsgHandled(TRUE);
			lResult = DragLeave();
			if(IsMsgHandled())
		...
		if (uMsg == WM_GRD_DRAGDROP)
		{
			SetMsgHandled(TRUE);
			DropTargetData *pData = reinterpret_cast<DropTargetData *>(wParam);
			DWORD *pdwEffect = reinterpret_cast<DWORD *>(lParam); 
			lResult = OnDrop(pData->pFmtEtc, *pData->pMedium, pdwEffect);
			if(IsMsgHandled())
				return TRUE;
		}
#endif
	END_MSG_MAP()
	BEGIN_UPDATE_UI_MAP(CGridCtrl)
	...
	{
#ifndef GRIDCONTROL_NO_CLIPBOARD
		UIEnable(ID_EDIT_COPY, GetSelectedCount() > 0);
		UIEnable(ID_EDIT_CUT, IsEditable() && GetSelectedCount() > 0);
